if app_value(:upgrade)
  if Kafo::Helpers.module_enabled?(@kafo, 'katello')
    ...
    Kafo::Helpers.log_and_say :info, "Upgrade Step: Update gpg key urls to support capsule isolation (this may take a while)...#{noop}"
    Kafo::Helpers.execute('foreman-rake katello:upgrades:2.2:update_gpg_key_urls') unless app_value(:noop)

    Kafo::Helpers.log_and_say :info, "Upgrade Step: Update repositories to specify metadata_expire (this may take a while)...#{noop}"
    Kafo::Helpers.execute('foreman-rake katello:upgrades:2.2:update_metadata_expire') unless app_value(:noop)
  end
